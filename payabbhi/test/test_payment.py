...
class TestPayment(unittest2.TestCase):
    ...
    def test_payment_transfer(self):
        ...
        transfer_url = '{0}/{1}/transfer'.format(self.payment_url, self.payment_id)
        responses.add(responses.POST, transfer_url, status=200,
                      body=transfer_result, match_querystring=True)
        response = self.client.payment.transfer(self.payment_id, data={'transfers':[{'amount':20,'currency':'INR','recipient_id':'recp_Za30i2k3p6blq3i1'},
